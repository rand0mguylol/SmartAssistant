...
public class KochLine : KochGenerator
{
  ...
  private void Update()
  {
    if (colorOnAudio)
    {
      if (useBufferOnColor)
      {
        _matInstance.SetColor("_TintColor", color * agent.GetAudioBandBuffer(audioBandMaterial) * emissionMultiplier);
        _matInstance.SetColor("_EmissionColor", color * agent.GetAudioBandBuffer(audioBandMaterial) * emissionMultiplier);
      }
      else
      {
        _matInstance.SetColor("_TintColor", color * agent.GetAudioBand(audioBandMaterial) * emissionMultiplier);
        _matInstance.SetColor("_EmissionColor", color * agent.GetAudioBand(audioBandMaterial) * emissionMultiplier);
      }
    }
    ...
    if (generationCount != 0)
    {
      ...
      {
        _lerpAudio[i] = agent.GetAudioBandBuffer(audioBand[i]);
